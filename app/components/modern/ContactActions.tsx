...
export const ContactActions: React.FC<ContactActionsProps> = ({ className = '', variant = 'primary', lang = 'en', label }) => {
    ...
    const primaryAction = useMemo(() => {
        ...
        if (primaryContact === 'WHATSAPP') {
            return {
                ...
                icon: (
                    <div className="w-8 h-8 md:w-10 md:h-10 rounded-lg gradient-accent flex items-center justify-center shadow-lg flex-shrink-0">
                        <svg className="w-4 h-4 md:w-5 md:h-5 transition-transform hover:scale-110" fill="white" viewBox="0 0 24 24">
    ...
    const baseClasses = "inline-flex items-center justify-center gap-1.5 sm:gap-2 px-4 sm:px-6 md:px-8 py-2 md:py-4 rounded-full font-semibold transition-all border";
